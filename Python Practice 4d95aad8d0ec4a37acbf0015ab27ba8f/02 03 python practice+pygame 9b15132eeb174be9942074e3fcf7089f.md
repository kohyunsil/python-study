...
# 화면 타이틀 설정
pygame.display.set_caption("Siri Pang") # 게임 이름
...
# 게임 종료 메시지 / TimeOut(시간초과로 실패), Mission Complete(미션성공), Game Over(공에 맞음,실패)
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        else: # 그 외의 모든 경우에는 속도를 증가(포물선효과를위한..)
            ball_val["to_y"] += 0.5
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "img_idx" : ball_img_idx + 1, #공의 이미지 인덱스 값
                        "to_x": -3, # x축 이동방향, -3 이면 왼쪽으로, 3이면 오른쪽으로
                        "to_y": -6, # y축 이동방향,
            ...
            continue    #안쪽 for문 조건이 맞지 않으면 continue. 바깥 for문계속 수행
        break # 안쪽 for문에서 break를 만나면 여기로 진입 가능. 2중 for문 한번에 탈출
    
    # 충돌된 공 or 무기 없애기
    if ball_to_remove > -1:
        del balls[ball_to_remove]  #리스트에서 이 위치에 있는 값 지워짐
        ball_to_remove = -1
